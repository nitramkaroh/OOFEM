...
void VectorAssembler :: vectorFromElement(FloatArray& vec, Element& element, TimeStep* tStep, ValueModeType mode) const { vec.clear(); }
  ...
  void VectorAssembler :: vectorFromElementSurface(FloatArray& vec, Element& element, TimeStep* tStep, ValueModeType mode) const { vec.clear(); }
...
void VectorAssembler :: locationFromElementSurface(IntArray& loc, Element& element, const UnknownNumberingScheme& s, IntArray* dofIds) const
{
    element.giveLocationArray(loc, s, dofIds);
}
...
void InternalForceAssembler :: vectorFromElementSurfaces(FloatArray& vec, Element& element, TimeStep* tStep, ValueModeType mode) const
{
    element.giveCharacteristicVector(vec, SurfaceInternalForcesVector, mode, tStep);
    //element.computeInternalForces(vec, tStep);
}
...
void TangentAssembler :: matrixFromElementSurfaces(FloatMatrix& mat, Element& element, TimeStep* tStep) const
{
    if ( this->rmode == TangentStiffness ) {
        element.giveCharacteristicMatrix(mat, SurfaceTangentStiffnessMatrix, tStep);
    }
    //element.computeTangentMatrix(mat, this->rmode, tStep);
}
