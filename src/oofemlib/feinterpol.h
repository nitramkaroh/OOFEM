...
public:
    ...
    int giveNumberOfVertices() const;
    inline const FloatArray *giveVertexCoordinates(int i) const {return &(elem->giveNode(i)->giveNodeCoordinates());}
  };
...
public:
FEIElementDeformedGeometryWrapper(const Element * elem) : FEICellGeometry() {
        this->elem = elem;
	this->mode = elem->giveDomain()->giveEngngModel()->giveFormulation();
	this->alpha = 1;
	this->tStep = NULL;
    }
