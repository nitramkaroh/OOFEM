...
void
EngngModel :: reduceTimeStep(TimeStep *tStep)
{
...
//
{
 ...
 #pragma omp critical
	...
	if( element->hasSurfaceEnergy()) {
	  va.vectorFromElementSurface(charVec, *element, tStep, mode);
	  if ( charVec.isNotEmpty() ) {
            if ( element->giveRotationMatrix(R) ) {
	        charVec.rotatedWith(R, 't');
            }
            va.locationFromElementSurface(loc, *element, s, & dofids);
	    answer.assemble(charVec, loc);
	    if ( eNorms ) {
	      eNorms->assembleSquared(charVec, dofids);
	    }
...
EngngModel :: assembleExtrapolatedForces(FloatArray &answer, TimeStep *tStep, CharType type, Domain *domain)
{
 ...
 #pragma omp parallel for shared(answer) private(R, charMatrix, charVec, loc, delta_u)
    ...
    for ( int i = 1; i <= nelems; i++ ) {
        ...
        element->giveCharacteristicMatrix(charMatrix, type, tStep->givePreviousStep());
        element->computeVectorOfPrescribed(VM_Incremental, tStep, delta_u);
	/*	FloatArray u;
	element->computeVectorOf(VM_Total, tStep, u);
	*/
