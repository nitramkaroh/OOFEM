...
void FloatMatrix :: copyRow(FloatArray &dest, int r) const
{
    int nc = this->giveNumberOfColumns();
#ifdef DEBUG
    if ( r < 1 || r > this->giveNumberOfRows() ) {
        OOFEM_ERROR("Row outside range (%d)", r);
    }
#endif

    dest.resize(nc);
     for ( int j = 1; j <= nc; j++ ) {
       dest.at(j) = this->at(r, j);
    }
...
void FloatMatrix :: beMatrixFormOfStrain(const FloatArray &aArray)
{
...
#  endif
    ...
    if ( aArray.giveSize() == 9 ) {
        ...
        this->at(2, 2) = aArray.at(2);
        this->at(3, 3) = aArray.at(3);
        this->at(2, 3) = aArray.at(4)/2;
        this->at(1, 3) = aArray.at(5)/2;
        this->at(1, 2) = aArray.at(6)/2;
        this->at(3, 2) = aArray.at(4)/2;
        this->at(3, 1) = aArray.at(5)/2;
        this->at(2, 1) = aArray.at(6)/2;
    }
}
