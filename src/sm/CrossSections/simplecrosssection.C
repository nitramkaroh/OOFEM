...
void
SimpleCrossSection :: giveSpatialStiffnessMatrix(FloatMatrix &answer,
                                               MatResponseMode rMode, GaussPoint *gp,
                                               TimeStep *tStep)
{
    StructuralMaterial *mat = dynamic_cast< StructuralMaterial * >( this->giveMaterial(gp) );
    ...
    MaterialMode mode = gp->giveMaterialMode();
    if ( mode == _3dMat ) {
        mat->giveSpatial3dMaterialStiffnessMatrix(answer, rMode, gp, tStep);
    } else if ( mode == _PlaneStress ) {
        mat->giveSpatialPlaneStressStiffMtrx(answer, rMode, gp, tStep);
    } else if ( mode == _PlaneStrain ) {
        mat->giveSpatialPlaneStrainStiffMtrx(answer, rMode, gp, tStep);
    } else if ( mode == _1dMat ) {
        mat->giveSpatial1dStressStiffMtrx(answer, rMode, gp, tStep);
    } else {
        OOFEM_ERROR( "unknown mode (%s)", __MaterialModeToString(mode) );
    }
}
...
void
SimpleCrossSection :: giveInitialStiffnessMatrix_Cauchy(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
{
    StructuralMaterial *mat = dynamic_cast< StructuralMaterial * >( this->giveMaterial(gp) );
    mat->giveInitialStiffnessMatrix_Cauchy(answer, rMode, gp, tStep);
}
