...
#include "../sm/Materials/gradientdamagematerialextensioninterface.h"
#include "node.h"
#include "material.h"
...
GradDpElement :: computeStressVectorAndLocalCumulatedStrain(FloatArray &answer, double &localCumulatedStrain, GaussPoint *gp, TimeStep *tStep)
{
    ...
    GradientDamageMaterialExtensionInterface *dpmat = static_cast< GradientDamageMaterialExtensionInterface * >(
    ...
    if ( nlGeo == 0 ) {
        ...
        dpmat->giveRealStressVectorGradientDamage(answer, localCumulatedStrain, gp, Epsilon, nlCumulatedStrain, tStep);
    } else if ( nlGeo == 1 ) {
        if ( elem->giveDomain()->giveEngngModel()->giveFormulation() == TL ) {
            FloatArray vF;
            this->computeDeformationGradientVector(vF, gp, tStep);
            dpmat->giveFirstPKStressVectorGradientDamage(answer, localCumulatedStrain, gp, vF, nlCumulatedStrain, tStep);
        } else {
            FloatArray vF;
            this->computeDeformationGradientVector(vF, gp, tStep);
            dpmat->giveCauchyStressVectorGradientDamage(answer, localCumulatedStrain, gp, vF, nlCumulatedStrain, tStep);
        }
    }
}
...
GradDpElement :: giveInternalForcesVector(FloatArray &answer, TimeStep *tStep, int useUpdatedGpRecord)
{
    ...
    for ( auto &gp: *elem->giveIntegrationRule(0) ) {
        GradientDamageMaterialExtensionInterface *dpmat = dynamic_cast< GradientDamageMaterialExtensionInterface * >(
            cs->giveMaterialInterface(GradientDamageMaterialExtensionInterfaceType, gp) );
        if ( !dpmat ) {
            OOFEM_ERROR("Material doesn't implement the required DpGrad interface!");
        }
...
GradDpElement :: computeStiffnessMatrix_ku(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
{
	...
	if(Dku.giveNumberOfRows()>0) {
	  DkuB.beProductOf(Dku, B);
	  FloatMatrix Nkm(Nk, true);
	  answer.plusProductUnsym(Nkm, DkuB, -dV);
	} else {
	  if(answer.giveNumberOfColumns() == 0) {
	    answer.resize(Nk.giveSize(), B.giveNumberOfColumns());
	  }
	}
...
GradDpElement :: computeStiffnessMatrix_kk(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
{
	...
	FloatMatrix Nkm(Nk, true);
        ...
        dpmat->givePDGradMatrix_kk(lStiff, rMode, gp, tStep);
        answer.plusProductUnsym(Nkm, Nkm, dV);
        if ( dpmat->giveAveragingType() == 0 || dpmat->giveAveragingType() == 1 ) {
            double l = lStiff.at(1, 1);
