...
void
NLStructuralElement :: computeDeformationGradientVector(FloatArray &answer, GaussPoint *gp, TimeStep *tStep, ValueModeType modeType)
{
  ...
  this->computeVectorOf({D_u, D_v, D_w}, VM_Incremental, tStep, uTest); // solution vector
...
NLStructuralElement :: computeCauchyStressVector(FloatArray &answer, GaussPoint *gp, TimeStep *tStep)
{
    ...
    FloatArray vF;
    this->computeDeformationGradientVector(vF, gp, tStep, VM_Incremental);
...
NLStructuralElement :: giveInternalForcesVector(FloatArray &answer, TimeStep *tStep, int useUpdatedGpRecord)
{
	...
	if ( this->domain->giveEngngModel()->giveFormulation() == AL ) { // Cauchy stress
	  ...
	  this->computeBmatrixAt(gp, B, tStep,1);
...
void
NLStructuralElement :: giveSurfaceInternalForcesVector(FloatArray &answer, TimeStep *tStep, int useUpdatedGpRecord)
{
    FloatMatrix B;
    FloatArray vStress, vStrain, u;
                                              ...
                                              MatResponseMode rMode, TimeStep *tStep)
{
    ...
    /* MeanDilatationalMethodElementExtensionInterface *meanDilInterface = static_cast<    MeanDilatationalMethodElementExtensionInterface* >( this->giveInterface( MeanDilatationalMethodElementExtensionInterfaceType) );
    // Check MeanDil interface
    if(meanDilInterface) {
      meanDilInterface->computeStiffnessMatrix(answer, rMode, tStep, this);
         // Check for F-bar interface      
    } else if(fbarInterface && fbarInterface->giveFbarFlag())  {
    fbarInterface->computeStiffnessMatrix(answer, rMode, tStep, this);
    } else {  */
	  ...
	  if ( nlGeometry == 0 ) {
	    ...
	    if ( this->domain->giveEngngModel()->giveFormulation() == AL ) { // Material stiffness dC/de
	      this->computeBHmatrixAt(gp, B, tStep);
	  ...
	  if ( this->domain->giveEngngModel()->giveFormulation() == AL ) {
	    ...
	    this->computeBHmatrixAt(gp, Bh,tStep,1);
	...
	if ( easInterface ) {
	  ...
	  for ( int j = 0; j < (int)integrationRulesArray.size(); j++ ) {
		...
		if ( this->domain->giveEngngModel()->giveFormulation() == AL ) { // Material stiffness dC/de
	      ...
	      if ( i != j ) {
		if ( nlGeometry == 0 ) {
		  this->computeBmatrixAt(gp, Bj, tStep, jStartIndx, jEndIndx);
		} else if ( nlGeometry == 1 ) {
		  if ( this->domain->giveEngngModel()->giveFormulation() == AL ) {
		    this->computeBmatrixAt(gp, Bj, tStep);
		  } else {
...
void
NLStructuralElement :: computeSurfaceStiffnessMatrix(FloatMatrix &answer,
                                              MatResponseMode rMode, TimeStep *tStep)
{
    StructuralCrossSection *cs = this->giveStructuralCrossSection();
    bool matStiffSymmFlag = cs->isCharacteristicMtrxSymmetric(rMode);
...
NLStructuralElement :: computeInitialStressMatrix(FloatMatrix &answer, TimeStep *tStep)
{
    ...
    for ( auto &gp : *this->giveDefaultIntegrationRulePtr() ) {
	    ...
	    FloatMatrix SiB;   
	    this->computeBmatrixAt(gp, B);
	    SiB.beProductOf(stress_ident,B);
            answer.plusProductSymmUpper( B, SiB, this->computeVolumeAround(gp) );
        }
    }
