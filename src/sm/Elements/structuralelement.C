...
void
StructuralElement :: computeSurfaceStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode,
                                            TimeStep *tStep)
// Computes numerically the stiffness matrix of the receiver.
{
    int iStartIndx, iEndIndx, jStartIndx, jEndIndx;
...
//
{
  ...
  void
StructuralElement :: giveSurfaceInternalForcesVector(FloatArray &answer,
                                              TimeStep *tStep, int useUpdatedGpRecord)
//
// returns nodal representation of real internal forces - necessary only for
// non-linear analysis.
...
//
{
    ...
    } else if ( mtrx == SurfaceTangentStiffnessMatrix ) {
        this->computeSurfaceStiffnessMatrix(answer, SecantStiffness, tStep);
...
//
{
    ...
    } else if ( ( mtrx == SurfaceInternalForcesVector ) && ( mode == VM_Total ) ) {
        this->giveSurfaceInternalForcesVector(answer, tStep);
