...
NlBeam_SM2 :: initializeFrom(InputRecord *ir)
{
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, pressure, _IFT_NlBeam_SM2_pressure);
    if(pressure != 0) {
      IR_GIVE_FIELD(ir, pressure_ltf, _IFT_NlBeam_SM2_pressureLTF);
    } else {
      pressure_ltf = 1;
    }
...
NlBeam_SM2 :: computeCenterlineStrainFromInternalForces(double M, double N, double curvature)
{
 ...
 void
 NlBeam_SM2 :: integrateAlongBeamAndGetJacobi(const FloatArray &fab, FloatArray &ub, FloatMatrix &jacobi, TimeStep *tStep)
 {
   if(tangentVector.giveSize()) {
 ...
 void
 NlBeam_SM2 :: integrateAlongStraightBeamAndGetJacobi(const FloatArray &fab, FloatArray &ub, FloatMatrix &jacobi, TimeStep *tStep)
 {
   ub.resize(3);
 ...
 */
 bool
 NlBeam_SM2 :: findLeftEndForcesLocal(FloatArray &ub_target, FloatArray &fab_loc, TimeStep *tStep)
 {
 ...
 */
 ...
 void
 NlBeam_SM2 :: construct_l(FloatArray &l, double phia, double L, double cB, double sB)
 {
   l.resize(3);
 ...
 */
 void
 NlBeam_SM2 :: findLeftEndForces(const FloatArray &u, FloatArray &fab, TimeStep *tStep)
 {
...
void
NlBeam_SM2 :: giveInternalForcesVectorPressure_from_u(FloatArray &answer, TimeStep *tStep, const FloatArray &u)
 {
   // solution vector
   answer.resize(6);
   double p = pressure * this->giveDomain()->giveFunction(pressure_ltf)->evaluate(tStep, VM_Total);
 ...
 void
 NlBeam_SM2 :: computeStiffnessMatrix_num(FloatMatrix &answer,FloatMatrix &answer_p, MatResponseMode rMode, TimeStep *tStep)
 {
   // solution vector
