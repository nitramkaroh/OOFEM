#include "../sm/Elements/Beams/nlbeam_mod.h"
#include "material.h"
...
namespace oofem {
REGISTER_Element(NlBeamMod);


NlBeamMod :: NlBeamMod(int n, Domain *aDomain) : NLStructuralElement(n, aDomain)
{
    numberOfDofMans = 2;
...
void
NlBeamMod :: updateYourself(TimeStep *tStep)
...
void
NlBeamMod :: giveDofManDofIDMask(int inode, IntArray &answer) const
{
    answer = {
        D_u, D_w, R_v
    };
...
// Returns the pitch of the receiver.
{
 ...
 IRResultType
NlBeamMod :: initializeFrom(InputRecord *ir)
{
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, NIP, _IFT_NlBeamMod_NIP);
    IR_GIVE_FIELD(ir, EA, _IFT_NlBeamMod_EA);
    IR_GIVE_FIELD(ir, EI, _IFT_NlBeamMod_EI);
    IR_GIVE_FIELD(ir, GAs, _IFT_NlBeamMod_GAs);
    IR_GIVE_OPTIONAL_FIELD(ir, RADIUS, _IFT_NlBeamMod_RADIUS);
    IR_GIVE_OPTIONAL_FIELD(ir, DEPTH, _IFT_NlBeamMod_DEPTH);
    IR_GIVE_OPTIONAL_FIELD(ir, pressure, _IFT_NlBeamMod_pressure);
    if(pressure != 0) {
      IR_GIVE_FIELD(ir, pressure_ltf, _IFT_NlBeamMod_pressureLTF);
    } else {
      pressure_ltf = 1;
    }

    IR_GIVE_OPTIONAL_FIELD(ir, coupling, _IFT_NlBeamMod_coupling);
    ...
    tangentVector.clear();  
    IR_GIVE_OPTIONAL_FIELD(ir, tangentVector, _IFT_NlBeamMod_tangentVector);
      
    if(tangentVector.giveSize()) {
      int n_cf = 0;
      IR_GIVE_OPTIONAL_FIELD(ir, n_cf,  _IFT_NlBeamMod_coordinateFlag);
      IR_GIVE_FIELD(ir, u_0, _IFT_NlBeamMod_u0);
      IR_GIVE_FIELD(ir, w_0, _IFT_NlBeamMod_w0);
      IR_GIVE_FIELD(ir, phi_0, _IFT_NlBeamMod_phi0);
      IR_GIVE_FIELD(ir, kappa_0, _IFT_NlBeamMod_kappa0);


      //      IR_GIVE_OPTIONAL_FIELD(ir, n_int, _IFT_NlBeamMod_nintervals);
	...
	IR_GIVE_FIELD(ir, curvedbeamLength, _IFT_NlBeamMod_curvedbeamLength);
      }
	...
	this->cf = CF_x;
	IR_GIVE_FIELD(ir, sx, _IFT_NlBeamMod_s);
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, beam_tol, _IFT_NlBeamMod_Tolerance);
    // maximum number of iterations at the beam level
    IR_GIVE_OPTIONAL_FIELD(ir, beam_maxit, _IFT_NlBeamMod_MaxIteration);
...
double
NlBeamMod :: computeMomentFromCurvature(double kappa)
{
   return EI*kappa;
}

double
NlBeamMod :: computeDerMomentFromCurvature(double kappa)
{
  return EI;
}

double
NlBeamMod :: computeCurvatureFromMoment(double M)
{
  return M/EI;
}
...
*/
double
NlBeamMod :: eval_kappa0(double x)
{  
...
*/
double 
NlBeamMod :: computeDeltaCurvatureFromInternalForces(double M, double N, double curvature)
{
  double delta_kappa;
  if(coupling) {
...
double 
NlBeamMod :: computeDerCurvatureMoment(double M, double N, double curvature)
{
  if(coupling) {
...
double 
NlBeamMod :: computeDerCurvatureNormalForce(double M, double N, double curvature)
{
  if(coupling) {
    return curvature/(EA);
  } else {
...
double 
NlBeamMod :: computeCenterlineStrainFromInternalForces(double M, double N, double curvature)
{
  if(coupling) {
 ...
 double 
 NlBeamMod :: computeDerStrainMoment(double M, double N, double curvature)
 {
   if(coupling) {
   return curvature/(EA);
   } else {
 ...
 double 
 NlBeamMod :: computeDerStrainNormalForce(double M, double N)
 {
   return 1./EA;
 }
 ...
 void
 NlBeamMod :: integrateAlongBeamAndGetJacobi(const FloatArray &fab, FloatArray &ub, FloatMatrix &jacobi, TimeStep *tStep)
 {
   if(tangentVector.giveSize()) {
 ...
 void
 NlBeamMod :: integrateAlongCurvedBeamAndGetJacobi(const FloatArray &fab, FloatArray &ub, FloatMatrix &jacobi)
 {
...
double
NlBeamMod :: computeChi_mid( double Xab, double Zab )
{
  double chi_mid = 0.;
  int iter = 0;
  double error = 10. * beam_tol;
  
     while ((iter==0) || (error>beam_tol && iter < beam_maxit && error ==error) ){
       iter++;
       //
       double f = chi_mid + ( Zab * cos(chi_mid) - Xab * sin(chi_mid) ) * (EA - Xab * cos(chi_mid) - Zab * sin(chi_mid) ) / (EA * GAs);
       double df = 1 + ( -Zab * EA * sin(chi_mid) + Xab * cos(chi_mid) * ( 4 * Zab * sin(chi_mid) - EA) + ( Zab * Zab - Xab * Xab ) * ( sin(chi_mid) * sin(chi_mid) - cos(chi_mid) * cos(chi_mid) ) ) / ( EA * GAs );
       //
       error = f/df;
       chi_mid -= error;
     }
 ...
 void
 NlBeamMod :: integrateAlongStraightBeamAndGetJacobi(const FloatArray &fab, FloatArray &ub, FloatMatrix &jacobi, TimeStep *tStep)
 {
   ub.resize(3);
   FloatArray du(3), dw(3), dphi(3), dM(3), dkappa(3), dphi_mid(3), dN_mid(3), dQ_mid(3), dlambda_mid(3), dchi_mid(3), dN_tilde(3), dQ_tilde(3), dlambda_tilde(3), ddchi(3);
   double Xab = fab.at(1), Zab = fab.at(2), Mab = fab.at(3);
   //@updated
   this->vM.at(1) = -Mab + Xab * w.at(1) - Zab * u.at(1);
   this->vN.at(1) = -Xab * cos(phi.at(1)) + Zab * sin(phi.at(1)); //need to be changed
   double dx = beamLength/NIP;
   //
   double chi_mid = computeChi_mid(Xab,Zab);
   double sin2chi_mid = sin(chi_mid) * cos(chi_mid) + cos(chi_mid) * sin(chi_mid);
   double cos2chi_mid = cos(chi_mid) * cos(chi_mid) - sin(chi_mid) * sin(chi_mid);
   dchi_mid.at(1) = ( EA * sin(chi_mid) - Xab * sin2chi_mid + Zab * cos2chi_mid) / ( EA * GAs - EA * ( Xab * cos(chi_mid) + Zab * sin(chi_mid) ) + ( Xab * Xab - Zab * Zab ) * cos2chi_mid +  2. * Xab * Zab * sin2chi_mid ) ;
   dchi_mid.at(2) =  ( -EA * cos(chi_mid) + Xab * cos2chi_mid + Zab * sin2chi_mid) / ( EA * GAs - EA * ( Xab * cos(chi_mid) + Zab * sin(chi_mid) ) + ( Xab * Xab - Zab * Zab ) * cos2chi_mid +  2. * Xab * Zab * sin2chi_mid ) ;
   //
   for (int i=2; i <= NIP+1; i++) {
     ...
     //
     double N_tilde = -Xab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) ) + Zab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid));
     FloatArray dphi_dchi(dphi_mid);
     dphi_dchi.subtract(dchi_mid); 
     //
     dN_tilde = (Xab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid)) + Zab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) )) * dphi_dchi;
     dN_tilde.at(1) -= (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid));
     dN_tilde.at(2) += (sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid));
     //
     double Q_tilde = -Xab *  ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid)) - Zab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) );
     dQ_tilde = (-Xab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) ) + Zab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid))) *  dphi_dchi ;
     dQ_tilde.at(1) -= (sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid));
     dQ_tilde.at(2) -= (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid));
     //
     double lambda_tilde = 1. + N_tilde/EA;
     //
     dlambda_tilde = dN_tilde;
     dlambda_tilde.times(1./EA);
     /*
     dlambda_tilde.at(1) = dN_tilde.at(1) / EA;
     dlambda_tilde.at(2) = dN_tilde.at(2) / EA;
     dlambda_tilde.at(3) = dN_tilde.at(3) / EA;
     */
          //
     // Delta chi
     double dchi = ( Q_tilde * lambda_tilde - GAs * chi_mid )/( GAs + N_tilde * lambda_tilde - Q_tilde * Q_tilde / EA );
     //
     for(int i = 1; i <= 3; i++) {
       ddchi.at(i) = ( ( dQ_tilde.at(i) * lambda_tilde + Q_tilde *  dlambda_tilde.at(i) - GAs * dchi_mid.at(i) ) * ( GAs + N_tilde * lambda_tilde - Q_tilde * Q_tilde / EA) - ( Q_tilde * lambda_tilde - GAs * chi_mid ) * ( dN_tilde.at(i) * lambda_tilde + N_tilde * dlambda_tilde.at(i) - 2. * Q_tilde * dQ_tilde.at(i) / EA) )  / ( ( GAs + N_tilde * lambda_tilde - Q_tilde * Q_tilde / EA) * ( GAs + N_tilde * lambda_tilde - Q_tilde * Q_tilde / EA) );
     }
     //
     chi_mid += dchi ;
     dchi_mid += ddchi;
     /*
     dchi_mid.at(1) += ddchi.at(1); 
     dchi_mid.at(2) += ddchi.at(2);
     dchi_mid.at(3) += ddchi.at(3);
     */
     //
     double N_mid = -Xab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid)) + Zab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid));
     vN.at(2*(i-1)) = N_mid;
     dN_mid =  ( Xab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid) )  + Zab * ( cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) )) * dphi_dchi;
     dN_mid.at(1) += - ( cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) );
     dN_mid.at(2) += ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid) );
     //
     double Q_mid = -Xab *  ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid)) - Zab * (cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) );
     dQ_mid = (-Xab * ( cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) ) + Zab * ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid) ) ) * dphi_dchi;
     dQ_mid.at(1) -= ( sin(phi_mid) * cos(chi_mid) - cos(phi_mid) * sin(chi_mid) );
     dQ_mid.at(2) -= ( cos(phi_mid) * cos(chi_mid) + sin(phi_mid) * sin(chi_mid) );
     //
     double lambda_mid =  1. + N_mid / EA;
     dlambda_mid.at(1) = dN_mid.at(1) / EA;
     dlambda_mid.at(2) = dN_mid.at(2) / EA;
     dlambda_mid.at(3) = dN_mid.at(3) / EA;
     //
     ...
     this->vN.at(2*i-1) = -Xab * cos(phi.at(i)) + Zab *  sin(phi.at(i)); //need to be changed
     dphi = dphi_mid;
 ...
 */
 bool
 NlBeamMod :: findLeftEndForcesLocal(FloatArray &ub_target, FloatArray &fab_loc, TimeStep *tStep)
 {
 ...
 */
 ...
 void
 NlBeamMod :: construct_T(FloatMatrix &T, const double phia)
 {
   T.resize(3,3);
 ...
 */
 ...
 void
   NlBeamMod :: construct_Tprime(FloatMatrix &T, const double phia)
 {
   T.resize(3,3);
 ...
 void
 NlBeamMod :: construct_l(FloatArray &l, double phia)
 {
   l.resize(3);
 ...
 void
 NlBeamMod :: construct_l(FloatArray &l, double phia, double L)
 {
   l.resize(3);
 ...
 void
 NlBeamMod :: construct_l(FloatArray &l, double phia, double L, double cB, double sB)
 {
   l.resize(3);
 ...
 void
 NlBeamMod :: construct_lprime(FloatArray &l, const double phia)
 {
   l.resize(3);
 ...
 */
 void
 NlBeamMod :: findLeftEndForces(const FloatArray &u, FloatArray &fab, TimeStep *tStep)
 {
 ...
 */
  ...
  void
 NlBeamMod :: giveInternalForcesVector(FloatArray &answer, TimeStep *tStep, int useUpdatedGpRecord)
 {
   // solution vector
   answer.resize(6);
...
void
NlBeamMod :: giveInternalForcesVectorPressure_from_u(FloatArray &answer, TimeStep *tStep, const FloatArray &u)
 {
   // solution vector
   answer.resize(6);
   double p = pressure * this->giveDomain()->giveFunction(pressure_ltf)->evaluate(tStep, VM_Total);
  ...
  void
  NlBeamMod :: giveInternalForcesVector_from_u(FloatArray &answer, TimeStep *tStep, const FloatArray &u)
 {
   // solution vector
   answer.resize(6);
 ...
 void
 NlBeamMod :: computeStiffnessMatrix_num(FloatMatrix &answer,FloatMatrix &answer_p, MatResponseMode rMode, TimeStep *tStep)
 {
   // solution vector
 ...
 */
 void
 NlBeamMod :: computeStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, TimeStep *tStep)
 {
...
Interface *NlBeamMod :: giveInterface(InterfaceType it)
{
    switch ( it ) {
...
void
NlBeamMod :: giveCompositeExportData(std::vector< VTKPiece > &vtkPieces, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, IntArray cellVarsToExport, TimeStep *tStep )
{
  if(tangentVector.giveSize()) {
...
void
NlBeamMod :: giveCompositeExportData_curved(std::vector< VTKPiece > &vtkPieces, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, IntArray cellVarsToExport, TimeStep *tStep )
{
...
void
NlBeamMod :: giveCompositeExportData_straight(std::vector< VTKPiece > &vtkPieces, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, IntArray cellVarsToExport, TimeStep *tStep )
{
    vtkPieces.resize(1);
...
void
NlBeamMod :: printOutputAt(FILE *file, TimeStep *tStep)
{
  if(tangentVector.giveSize()) {
...
void
NlBeamMod :: printOutputAt_StraightBeam(FILE *file, TimeStep *tStep)
{
...
void
NlBeamMod :: printOutputAt_CurvedBeam(FILE *file, TimeStep *tStep)
{
...
FILE *
NlBeamMod :: giveOutputStream(TimeStep *tStep)
{
    FILE *answer;
