...
Quad1PlaneStrain :: Quad1PlaneStrain(int n, Domain *aDomain) :
    ...
    HuertaErrorEstimatorInterface(), MeanDilatationalMethodElementExtensionInterface(aDomain)
{
    numberOfDofMans  = 4;
...
void
Quad1PlaneStrain :: computeBmatrixAt(GaussPoint *gp, FloatMatrix &answer, TimeStep *tStep, int li, int ui)
//
...
// r = ( u1,v1,u2,v2,u3,v3,u4,v4)
{
    ...
    this->interp.evaldNdx( dN, gp->giveNaturalCoordinates(), *this->giveCellGeometryWrapper(tStep) );
...
void
Quad1PlaneStrain :: computeBHmatrixAt(GaussPoint *gp, FloatMatrix &answer, TimeStep *tStep, double alpha)
// Returns the [5x8] displacement gradient matrix {BH} of the receiver,
// evaluated at gp.
{
    FloatMatrix dnx;
    this->interp.evaldNdx( dnx, gp->giveNaturalCoordinates(), *this->giveCellGeometryWrapper(tStep, alpha));
...
Quad1PlaneStrain :: initializeFrom(InputRecord *ir)
{
    ...
    result = MeanDilatationalMethodElementExtensionInterface :: initializeFrom(ir);
    if ( result != IRRT_OK ) {
      return result;
    }
