...
LBrickGradPolyconvex :: interpolation(1, 2);

LBrickGradPolyconvex :: LBrickGradPolyconvex(int n, Domain *aDomain) : LSpace(n, aDomain), BaseMicromorphicElement()
    // Constructor.
{
  int index = 0;
  for(int iNode = 1; iNode <=8; i++) {
    for( int iDof = 1; iDof <= 12; jDof++ ) {
      index++;
      if(iDof <= 3) {
	displacementDofsOrdering.append(index);
      } else {
	micromorphicDofsOrdering.append(index);
      }
...
{
  answer = {D_u, D_v, M_X11, M_X22, M_X33, M_X23, M_X13, M_X12, M_X32, M_X31, M_X21};
}
...
LBrickGradPolyconvex :: giveDofManDofIDMask_m(IntArray &answer)
{
  ...
  answer = {X11, M_X22, M_X33, M_X23, M_X13, M_X12, M_X32, M_X31, M_X21};
