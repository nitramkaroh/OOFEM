...
#include "../sm/Elements/Micromorphic/GradientPolyconvex/lcbrickgradpolyconvex.h"
...
namespace oofem {
REGISTER_Element(LCBrickGradPolyconvex);
  
FEI3dHexaLin LCBrickGradPolyconvex :: micro_interpolation;

LCBrickGradPolyconvex :: LCBrickGradPolyconvex(int n, Domain *aDomain) : QSpace(n, aDomain), BaseMicromorphicElement()
    // Constructor.
{
  int index = 0;
  for(int iNode = 1; iNode <=20; iNode++) {
...
void 
LCBrickGradPolyconvex :: computeMicromorphicNMatrixAt(GaussPoint *gp,FloatMatrix &answer)
{
    FloatArray n;
    this-> micro_interpolation.evalN( n, gp->giveNaturalCoordinates(), FEIElementGeometryWrapper(this) );
    answer.beNMatrixOf(n, 9);
}


void
LCBrickGradPolyconvex :: computeMicromorphicBMatrixAt(GaussPoint *gp, FloatMatrix &answer)         
{
    FloatMatrix dNdx; 
    this->micro_interpolation.evaldNdx( dNdx, gp->giveNaturalCoordinates(), FEIElementGeometryWrapper(this) );
...
void
LCBrickGradPolyconvex :: computeMicromorphicVars(FloatArray &micromorphicVar,FloatArray &micromorphicVarGrad, IntArray IdMask_m, GaussPoint *gp, TimeStep *tStep)
{
  FloatMatrix N_m, B_m;
  FloatArray u_m;
...
void
LCBrickGradPolyconvex :: giveDofManDofIDMask(int inode, IntArray &answer) const
{
  if(inode <= 8) {
    answer = {D_u, D_v, D_w};
  }
...
void
LCBrickGradPolyconvex :: giveDofManDofIDMask_u(IntArray &answer)
{
  answer = {D_u, D_v, D_w};
}
...
void
LCBrickGradPolyconvex :: giveDofManDofIDMask_m(IntArray &answer)
{
...
void
LCBrickGradPolyconvex ::  postInitialize() 
{
