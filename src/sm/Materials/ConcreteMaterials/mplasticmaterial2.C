                                                   ...
                                                   TimeStep *tStep)
{
    ...
    // transform stress vector to effective stress vector
    double omega = computeDamage(gp, strainSpaceHardeningVariables, tStep);    
    stressVector.times(1./(1.-omega));
...
void
MPlasticMaterial2 :: giveStiffnessCorrection(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{ 	
	/// contribution of damage to the stiffness matrix
    MPlasticMaterial2Status *status = static_cast< MPlasticMaterial2Status * >( this->giveStatus(gp) );
    double tempDamage = status->giveTempDamage();
    ...
    IntArray activeConditionMap = status->giveTempActiveConditionMap();
    ...
    this->give_dLambda_dEps_Matrix(dLam_dEps, mode, gp, tStep);
    this->computeReducedHardeningVarsLamGradient(dKappa_dLam, gp, actSurf, activeConditionMap, effectiveStress, strainSpaceHardeningVariables, gamma);
    ...
    double damagePrime = compute_dDamage_dKappa(gp, strainSpaceHardeningVariables, tStep);
    FloatArray dKdE;
    dKappa_dEps.copyRow(dKdE,1);
      //    dKdE.beRowOf(dKappa_dEps, 1);
    answer.beDyadicProductOf(effectiveStress, dKdE);    
    answer.times(damagePrime);
}

	
void
MPlasticMaterial2 :: give_dLambda_dEps_Matrix(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    //
    // returns receiver material matrix for given reached state
    // described by temp variables.
...
double
MPlasticMaterial2 :: compute_dDamage_dKappa(GaussPoint *gp, const FloatArray &strainSpaceHardeningVariables, TimeStep *tStep)
{
  return 0;
} 
...
MPlasticMaterial2Status :: MPlasticMaterial2Status(int n, Domain *d, GaussPoint *g, int statusSize) :
    ...
    gamma(), tempGamma(), damage(0.), tempDamage(0.)
{ }
