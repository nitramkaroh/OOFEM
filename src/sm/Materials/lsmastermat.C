...
LargeStrainMasterMaterial :: initializeFrom(InputRecord *ir)
{
    ...
    //@todo testing stuff, to be removed
    initialStrain.resize(0);
    IR_GIVE_OPTIONAL_FIELD(ir, initialStrain, _IFT_LargeStrainMasterMaterial_InitialStrain);
...
void
LargeStrainMasterMaterial :: giveRealStressVector_PlaneStress(FloatArray &answer, GaussPoint *gp, const FloatArray &redvE, TimeStep *tStep)
{
    ...
    FloatArray redvEpr(redvE);
    if(initialStrain.giveSize() != 0) {
      redvEpr.add(initialStrain);
    }
    
    //store of deformation gradient into 3x3 matrix
    StructuralMaterial :: giveFullSymVectorForm(vE, redvEpr, _PlaneStress);
    ...
    FloatMatrix aPP, aTL;
    this->giveTransformationMatrices_PlaneStressAnalyticalM_2_dSdE(aPP,aTL, C, SethHillStress, eVals, eVecs);
    /*    FloatMatrix redPP;
...
LargeStrainMasterMaterial :: giveFirstPKStressVector_PlaneStress(FloatArray &answer, GaussPoint *gp, const FloatArray &redvF, TimeStep *tStep)
{
    ...
    FloatArray vP;
    vP.beTProductOf(PP, redvSethHillStress);
...
void LargeStrainMasterMaterial :: giveFirstPKStressVector_Membrane2d(FloatArray &answer, GaussPoint *gp, const FloatArray &redvF, TimeStep *tStep)
{
    LargeStrainMasterMaterialStatus *status = static_cast< LargeStrainMasterMaterialStatus * >( this->giveStatus(gp) );
    this->initTempStatus(gp);
...
void
LargeStrainMasterMaterial :: givePlaneStressStiffMtrx(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    LargeStrainMasterMaterialStatus *status = static_cast< LargeStrainMasterMaterialStatus * >( this->giveStatus(gp) );
    StructuralMaterial *sMat = static_cast< StructuralMaterial * >( domain->giveMaterial(slaveMat) );
    FloatArray eVals, vE;
...
LargeStrainMasterMaterial :: givePlaneStressStiffMtrx_dPdF(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    ...
    FloatArray redvSethHillStress(status->giveTempStressVector()), redvP(status->giveTempPVector());
    FloatArray vSethHillStress, vP;

    if(redvP.giveSize() != 9) {
      StructuralMaterial :: giveFullVectorForm(vP, redvP, _PlaneStress);
    } else {
      vP = redvP;
    }
    if(redvSethHillStress.giveSize() != 6) {
          StructuralMaterial :: giveFullSymVectorForm(vSethHillStress, redvSethHillStress, _PlaneStress);
    } else {
      vSethHillStress = redvSethHillStress;
    }
...
{   
    ...
    answer.at(1, 1) = S.at(1,1);
    answer.at(1, 3) = S.at(1,2);    
    answer.at(2, 2) = S.at(2,2);
    answer.at(2, 4) = S.at(1,2);
    answer.at(3, 1) = S.at(2,1);
    answer.at(3, 3) = S.at(2,2);
    answer.at(4, 2) = S.at(1,2);
    answer.at(4, 4) = S.at(1,1);
}
...
{  
  ...
  for (int k = 1; k <= 2; k++) {
	...
	for (int n = 1; n<=2; n++) {
	  for (int i = 1; i <= 2; i++) {
	    if(k == l ) {
	      fPP.at(giveSymVI(k,l),giveVI(m,n)) += 0.5 * d.at(i) * N.at(k,i) * N.at(l,i) * M.at(giveVI(i,i),giveVI(m,n));
	    } else if(k < l) {
	      fPP.at(giveSymVI(k,l),giveVI(m,n)) += d.at(i) * N.at(k,i) * N.at(l,i) * M.at(giveVI(i,i),giveVI(m,n));
	    }

	    fTL.at(giveVI(k,l),giveVI(m,n)) += 1./4.*f.at(i)*dzeta.at(i,i)*M.at(giveVI(i,i),giveVI(k,l))*M.at(giveVI(i,i),giveVI(m,n));
	    for (int j  = 1; j <= 2; j++) {
	      if(j != i) {
		if( k == l) {
		  fPP.at(giveSymVI(k,l),giveVI(m,n)) += theta.at(i,j)*N.at(k,i)*N.at(l,j)*M.at(giveVI(i,j),giveVI(m,n));
		} else if(k < l) {
		  fPP.at(giveSymVI(k,l),giveVI(m,n)) += 2. * theta.at(i,j)*N.at(k,i)*N.at(l,j)*M.at(giveVI(i,j),giveVI(m,n));
		}
		fTL.at(giveVI(k,l),giveVI(m,n)) += 2.*ksi.at(i,j) * ( dzeta.at(i,j)*(M.at(giveVI(i,j),giveVI(k,l))*M.at(giveVI(j,j),giveVI(m,n)) + M.at(giveVI(j,j),giveVI(k,l))*M.at(giveVI(i,j),giveVI(m,n))) + dzeta.at(j,j)*M.at(giveVI(i,j),giveVI(k,l))*M.at(giveVI(i,j),giveVI(m,n) ));
		for (int q = 1; q <= 2; q++) {
		  if(q == i) {
		    continue;
		  } else if(q == j) {
		    ...
		    fTL.at(giveVI(k,l),giveVI(m,n)) += 2.*eta*dzeta.at(i,j)*M.at(giveVI(i,q),giveVI(k,l))*M.at(giveVI(j,q),giveVI(m,n));
		  }
		}
	      }
	    }
  ...
  IntArray nI,sI; 
  StructuralMaterial :: giveVoigtVectorMask(nI, _PlaneStress);
  StructuralMaterial :: giveVoigtSymVectorMask(sI, _PlaneStress);
  
  PP.beSubMatrixOf(fPP, sI, nI);
  TL.beSubMatrixOf(fTL, nI, nI);  
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_huhu(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &F, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{  
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_PlaneStress_dSdE(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{  
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_PlaneStress2(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &F, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{   
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_PlaneStress2_dSdE(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{   
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_PlaneStressAnalyticalM_2(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &Fm, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{
...
void
LargeStrainMasterMaterial :: giveTransformationMatrices_PlaneStressAnalyticalM_2_dSdE(FloatMatrix &PP,FloatMatrix &TL, const FloatMatrix &C, const FloatMatrix &SethHillStress, const FloatArray &lam, const FloatMatrix &N)
{
