...
MisesMatGrad :: givePlaneStrainStiffMtrx(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    ...
    const FloatArray &fullTrialStressDev = status->giveTrialStressDev();
    double trialS = computeStressNorm(fullTrialStressDev);
    FloatArray trialStressDev;
    StructuralMaterial :: giveReducedSymVectorForm(trialStressDev, fullTrialStressDev, gp->giveMaterialMode());
    // volumetric stress
    ...
    if ( tempDamage > damage ) {
        const FloatArray &fullEffStress = status->giveTempEffectiveStress();
	FloatArray effStress;
	StructuralMaterial :: giveReducedSymVectorForm(effStress, fullEffStress, gp->giveMaterialMode());
        double nlKappa = status->giveNonlocalCumulatedStrain();
...
MisesMatGrad :: givePlaneStrainKappaMatrix(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
	...
	answer.at(1, 4) = trialStressDev.at(6);
...
void
MisesMatGrad :: giveRealStressVectorGradientDamage(FloatArray &answer1, double &answer2, GaussPoint *gp, const FloatArray &reducedStrain, double nonlocalCumulatedStrain, TimeStep *tStep)
{
    ...
    MisesMat :: performPlasticityReturn(gp, reducedStrain);
...
MisesMatGrad :: giveNonlocalInternalForces_N_factor(double &answer, double nlDamageDrivingVariable, GaussPoint *gp, TimeStep *tStep)
{
    ...
    answer = nlDamageDrivingVariable - localCumulativePlasticStrain;
}
