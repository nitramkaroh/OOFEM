...
CamClayMat :: initializeFrom(InputRecord *ir)
{
    ...
    //OOFEM_WARNING("Initializing CamClay, theta = %lf", theta);
	
    yieldTolerance = 1.e-9;
    IR_GIVE_OPTIONAL_FIELD(ir, yieldTolerance, _IFT_CamClayMat_yieldTol);
    plasticStrainTolerance = 1.e-9;
...
CamClayMat :: performPlasticityReturn(GaussPoint *gp, const FloatArray &totalStrain, TimeStep *tStep)
{
    ...
    if (yieldValue > 0.) {
      ...
      double rIP = nextPc - theta*M*M/3*deltaLambda*(2 * nextP - nextPc)*nextPc - pc;
	...
	rSigmaByP.add(doubleDerivative);
	rSigmaByP.times(deltaLambda);
	...
	double rFByP = -M*M * nextP;
	
	//R^p_,sigma
	FloatArray rPBySigma;
	rPBySigma.resize(6);
	rPBySigma.at(1) = (2. / 9.)*M*M*theta*deltaLambda*nextPc;
	rPBySigma.at(2) = (2. / 9.)*M*M*theta*deltaLambda*nextPc;
	rPBySigma.at(3) = (2. / 9.)*M*M*theta*deltaLambda*nextPc;
	
	//R^p_,f
	double rPByF;
	rPByF = -theta*M*M/3*(2 * nextP - nextPc)*nextPc; 
	
	//R^p_,p
	double rPByP =  1.0 - 2 *M*M/3*theta*deltaLambda*(nextP-nextPc);
	...
	//Ri_p
	nextP = (-1. / 3.)*(nextStress.at(1) + nextStress.at(2) + nextStress.at(3));
	rIP = nextPc - theta*M*M/3*deltaLambda*(2 * nextP - nextPc)*nextPc - pc;
	
	//6) check for convergence
	iterationNumber++;
	double strainError = sqrt(rISigma.dotProduct(rISigma));
	convergence = (fabs(rIF) < yieldTolerance && strainError < plasticStrainTolerance && fabs(rIP) < pcTolerance);
	
	//if (iterationNumber % 5 == 0 || convergence) {
	//	OOFEM_WARNING("After %d iterations, Element number %d, gp %d, yieldError = %lf, strainError = %lf, pcError = %lf", iterationNumber, gp->giveElement()->giveNumber(), gp->giveNumber(), fabs(rIF), strainError, fabs(rIP));
	//}
                                            ...
                                            TimeStep *tStep)
{
    ...
    if (tempLambda - lastLambda > 0) {
      ...
      //R^p_,p
      double rPByP = 1. - 2. * theta*deltaLambda*M*M/3*(nextP - nextPc);
      ...
      FloatArray CN1, N2C;
      CN1.beProductOf(cMatrix,upperLeftBracket);
      N2C.beProductOf(cMatrix, upperRightBracket);    
      //      upperBrackets.beDyadicProductOf(upperLeftBracketTimesC, upperRightBracket);
      FloatMatrix upper;
      //      upper.beProductOf(upperBrackets, cMatrix);
      upper.beDyadicProductOf(CN1, N2C);
      ...
      lowerRightBracketTimesC.beProductOf(cMatrix, lowerRightBracket);
