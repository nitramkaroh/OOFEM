...
#include "../sm/Materials/Micromorphic/Microdil/microdilmaterial_elastic.h"
...
namespace oofem {
  REGISTER_Material(Microdilmaterial_Elastic);

Microdilmaterial_Elastic :: Microdilmaterial_Elastic(int n, Domain *d) :IsotropicLinearElasticMaterial(n, d), MicromorphicMaterialExtensionInterface(d)
{
  Hk = Ak = 0.;
...
Microdilmaterial_Elastic :: ~Microdilmaterial_Elastic()
{ }



void
Microdilmaterial_Elastic :: giveStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
//
...
void
Microdilmaterial_Elastic :: giveMicromorphicMatrix_dSigdUgrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
...
void
Microdilmaterial_Elastic :: giveMicromorphicMatrix_dSigdPhi(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
 MicromorphicMaterialStatus *status = static_cast< MicromorphicMaterialStatus * >( this->giveStatus(gp) );    
...
void
Microdilmaterial_Elastic :: giveMicromorphicMatrix_dSdUgrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
 FloatArray tempStrain, dEeqdE;
...
void
Microdilmaterial_Elastic :: giveMicromorphicMatrix_dSdPhi(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
  MaterialMode matMode = gp->giveMaterialMode();
  if(matMode == _3dMat){
    answer.resize(3,3);
...
void
Microdilmaterial_Elastic :: giveMicromorphicMatrix_dMdPhiGrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
  MaterialMode matMode = gp->giveMaterialMode();
  if(matMode == _3dMat){
    answer.resize(3,3);
...
void
Microdilmaterial_Elastic :: giveGeneralizedStressVectors (FloatArray &sigma, FloatArray &s, FloatArray &S, GaussPoint *gp, const FloatArray &totalStrain, const FloatArray &micromorphicVar, const FloatArray micromorphicVarGrad, TimeStep *tStep)
{
...
double
Microdilmaterial_Elastic :: computeEquivalentStrain(const FloatArray &strain, MaterialMode matMode)
{
...
void
Microdilmaterial_Elastic :: compute_dEeqdE(FloatArray &answer, const FloatArray &strain, MaterialMode matMode)
{
   if(this->equivStrainType == EST_MicroDil){
      if(matMode == _3dMat){
	answer = {1, 1, 1, 0, 0, 0};
...
void
Microdilmaterial_Elastic :: compute_d2EeqdE2(FloatMatrix &answer, const FloatArray &strain, MaterialMode matMode)
{
   if(this->equivStrainType == EST_MicroDil){
      answer.resize(0,0);
    } else if(this->equivStrainType == EST_MicroStrain) {
...
void
Microdilmaterial_Elastic :: giveInputRecord(DynamicInputRecord &input)
{
...
IRResultType
Microdilmaterial_Elastic :: initializeFrom(InputRecord *ir)
{
    IRResultType result;                             // Required by IR_GIVE_FIELD macro
    IsotropicLinearElasticMaterial :: initializeFrom(ir);
    int epsEq = 0;

    IR_GIVE_OPTIONAL_FIELD(ir, epsEq, _IFT_Microdilmaterial_Elastic_equivstraintype);
...
int
Microdilmaterial_Elastic :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
