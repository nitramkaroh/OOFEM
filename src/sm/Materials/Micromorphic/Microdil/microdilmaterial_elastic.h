...
#ifndef microdilmaterial_elastic_h
#define microdilmaterial_elastic_h
...
//@{
#define _IFT_Microdilmaterial_Elastic_Name "microdil_elastic"
#define _IFT_Microdilmaterial_Elastic_equivstraintype "equivstraintype"
 ...
 */
class Microdilmaterial_Elastic : public IsotropicLinearElasticMaterial, MicromorphicMaterialExtensionInterface
{
protected:
/// Type characterizing the micromorphic equivalent strain measure.
    enum EquivStrainType { EST_MicroDil, EST_MicroStrain};
    /// Parameter specifying the definition of equivalent strain.
    EquivStrainType equivStrainType;

public:
    Microdilmaterial_Elastic(int n, Domain * d);
    virtual ~Microdilmaterial_Elastic();

    // definition
    virtual const char *giveInputRecordName() const { return _IFT_Microdilmaterial_Elastic_Name; }
    virtual const char *giveClassName() const { return "Microdilmaterial_Elastic"; }
    ...
    double  computeEquivalentStrain(const FloatArray &strain, MaterialMode mode);
    void compute_dEeqdE(FloatArray &answer, const FloatArray &strain, MaterialMode mode);
    void compute_d2EeqdE2(FloatMatrix &answer, const FloatArray &strain, MaterialMode mode);
