...
#include "../sm/Materials/Micromorphic/GradientPolyconvex/gradientpolyconvexmaterialTest.h"
...
namespace oofem {
  REGISTER_Material(GradientPolyconvexMaterialTest);

GradientPolyconvexMaterialTest :: GradientPolyconvexMaterialTest(int n, Domain *d) :IsotropicLinearElasticMaterial(n, d), MicromorphicMaterialExtensionInterface(d)
{
  Hk = Ak = 0.;
  alpha = 1.e3;
  gamma = 1.e-5;
...
GradientPolyconvexMaterialTest :: ~GradientPolyconvexMaterialTest()
{ }



void
GradientPolyconvexMaterialTest :: giveStiffnessMatrix(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
//
...
void
GradientPolyconvexMaterialTest :: giveMicromorphicMatrix_dSigdUgrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    GradientPolyconvexMaterialTestStatus *status = static_cast< GradientPolyconvexMaterialTestStatus * >( this->giveStatus(gp) );
	...
	for (int p = 1; p <= 3; p++) {
	  for (int q = 1; q <= 3; q++) {
	    ...
	    answer.at(giveVI(i,j),giveVI(p,q)) += alpha * (4. * ( vB.at(giveVI(i,p)) * delta.at(giveVI(j,q)) + vF.at(giveVI(i,q)) *  vF.at(giveVI(p,j))) * (normC_tC1*normC_tC1) + 4. * vC_tC1.at(giveVI(j,q)) * delta.at(giveVI(i,p)) );
	    /// part with J^-1
...
void
GradientPolyconvexMaterialTest :: giveMicromorphicMatrix_dSigdPhi(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
...
void
GradientPolyconvexMaterialTest :: giveMicromorphicMatrix_dSdUgrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
    GradientPolyconvexMaterialTestStatus *status = static_cast< GradientPolyconvexMaterialTestStatus* >( this->giveStatus(gp) );
...
void
GradientPolyconvexMaterialTest :: giveMicromorphicMatrix_dSdPhi(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
...
void
GradientPolyconvexMaterialTest :: giveMicromorphicMatrix_dMdPhiGrad(FloatMatrix &answer, MatResponseMode mode, GaussPoint *gp, TimeStep *tStep)
{
  MaterialMode matMode = gp->giveMaterialMode();
  if (matMode == _PlaneStrain) {
    answer.resize(10,10);
...
void
GradientPolyconvexMaterialTest :: giveFiniteStrainGeneralizedStressVectors(FloatArray &vP, FloatArray &s, FloatArray &M, GaussPoint *gp, const FloatArray &vF, const FloatArray &micromorphicVar, const FloatArray micromorphicVarGrad, TimeStep *tStep)
{
...
void
GradientPolyconvexMaterialTest :: giveFiniteStrainGeneralizedStressVectors_3d(FloatArray &vP, FloatArray &s, FloatArray &M, GaussPoint *gp, const FloatArray &vF, const FloatArray &micromorphicVar, const FloatArray micromorphicVarGrad, TimeStep *tStep)
{
    ...
    double normC_tC1;
    FloatArray arb1, arb2, vC_tC1;
    FloatMatrix C, dCdF, C_tC1;
    C.beTProductOf(F,F);
    this->compute_dC_dF(dCdF,vF);
...
void
GradientPolyconvexMaterialTest :: giveFiniteStrainGeneralizedStressVectors_PlaneStrain(FloatArray &vP, FloatArray &s, FloatArray &M, GaussPoint *gp, const FloatArray &reducedvF, const FloatArray &reducedMicromorphicVar, const FloatArray micromorphicVarGrad, TimeStep *tStep)
{
...
void
GradientPolyconvexMaterialTest :: compute_dC_dF(FloatMatrix &dCdF,const FloatArray &vF)
{
...
FloatMatrix&
GradientPolyconvexMaterialTest :: givetC1(TimeStep *tStep)
 {
...
FloatMatrix &
GradientPolyconvexMaterialTest :: givetC2(TimeStep *tStep)
 {
   double t = tStep->giveIntrinsicTime();
   double tEps = (t+1) * eps;
...
IRResultType
GradientPolyconvexMaterialTest :: initializeFrom(InputRecord *ir)
{
    ...
    IR_GIVE_FIELD(ir, eps, _IFT_GradientPolyconvexMaterialTest_eps);
    IR_GIVE_OPTIONAL_FIELD(ir, alpha, _IFT_GradientPolyconvexMaterialTest_alpha );
    gamma = 1;
    IR_GIVE_OPTIONAL_FIELD(ir, gamma, _IFT_GradientPolyconvexMaterialTest_gamma );
...
int
GradientPolyconvexMaterialTest :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
  ...
  GradientPolyconvexMaterialTestStatus :: GradientPolyconvexMaterialTestStatus(int n, Domain *d, GaussPoint *g, bool sym) : MicromorphicMaterialStatus(n, d, g, sym)
{
